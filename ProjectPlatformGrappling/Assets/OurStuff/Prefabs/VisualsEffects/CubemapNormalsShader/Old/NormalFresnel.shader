Shader "Instanced/NormalFresnel" {
	Properties{
		...
		SubShader{
			...
			void surf(Input IN, inout SurfaceOutputStandard o) {
				...
				float3 nor = normalize(IN.worldNormal)*2.0f;
				nor = nor*0.5f;

				fixed4 c = tex2D(_MainTex, nor) * UNITY_ACCESS_INSTANCED_PROP(_Color); // Texture mapped STANDARD
